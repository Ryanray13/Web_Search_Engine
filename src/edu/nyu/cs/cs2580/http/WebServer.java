...
public class WebServer extends NanoHTTPD {
  ...
  protected static void registerPluginForMimeType(String[] indexFiles,
      String mimeType, Map<String, String> commandLineOptions) {
    if (mimeType == null) {
      return;
    }
  ...
  private Response respond(Map<String, String> headers, IHTTPSession session,
    ...
    if (!htmlUri) {
      ...
      if (queryStr.equals(""))
        ...
        uri = "http://" + headers.get("host") + uri + "?"
            + encodeUri(queryStr.substring(1));
      ...
      return createResponse(
          ...
          new ByteArrayInputStream(((ByteArrayOutputStream) exchange
    ...
    if (f.isDirectory()) {
      // First look for index files (index.html, index.htm, etc) and if none
      // found, list the directory if readable.
  ...
  protected Response getInternalErrorResponse(String s) {
    return createResponse(Response.Status.INTERNAL_ERROR,
        NanoHTTPD.MIME_PLAINTEXT, "INTERNAL ERRROR: " + s);
  }
    ...
    try {
      ...
      if (range != null && startFrom >= 0) {
        ...
        if (etag.equals(header.get("if-none-match")))
          ...
          res = createResponse(Response.Status.OK, mime, new FileInputStream(
              file));
